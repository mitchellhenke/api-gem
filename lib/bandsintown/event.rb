module Bandsintown
  class Event < Base
    ...
    def self.search(options = {})
      events = []
      request_and_parse(:get, 'search', options).each { |event| events << Bandsintown::Event.build_from_json(event) }
      events
    end
    ...
    def self.daily
      events = []
      request_and_parse(:get, 'daily').each { |event| events << Bandsintown::Event.build_from_json(event) }
      events
    end
    ...
    def self.recommended(options = {})
      events = []
      request_and_parse(:get, 'recommended', options).each { |event| events << Bandsintown::Event.build_from_json(event) }
      events
    end
    ...
    def self.on_sale_soon(options = {})
      events = []
      request_and_parse(:get, 'on_sale_soon', options).each { |event| events << Bandsintown::Event.build_from_json(event) }
      events
    end
    ...
    def self.create(options = {})
      event_data = {
        artists: parse_artists(options[:artists]),
        venue: parse_venue(options[:venue]),
        datetime: parse_datetime(options[:datetime]),
        on_sale_datetime: parse_datetime(options[:on_sale_datetime]),
        ticket_url: options[:ticket_url],
        ticket_price: options[:ticket_price]
      }.reject { |_k, v| v.blank? }

      response = request_and_parse(:post, '', event: event_data)
    ...
    #
    def cancel
      fail StandardError.new('event cancellation requires a bandsintown_id') if @bandsintown_id.blank?
