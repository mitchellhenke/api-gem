module Bandsintown
  class Artist < Base
    ...
    def self.create(options)
      build_from_json(request_and_parse(:post, '', artist: options))
    end
    ...
    def cancel_event(event_id)
      fail StandardError.new('event cancellation requires a bandsintown_id') if event_id.blank?
    ...
    def build_bandsintown_url
      if @name
        ...
        name.gsub!('&', 'And')
        name.gsub!('+', 'Plus')
        name = name.split.map(&:capitalize).join if name =~ /\s/
        name.gsub!('/', CGI.escape('/'))
        name.gsub!('?', CGI.escape('?'))
